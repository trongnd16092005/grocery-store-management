                    ...
                    <div class="card border-0 shadow-sm"><div class="card-body p-4">
                        ...
                        <form method="post" action="${pageContext.request.contextPath}/discount-codes">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-6"><label class="form-label fw-semibold">Nhóm khách áp dụng</label><select class="form-select" name="customerTypeScope"><option value="ALL" ${empty editingCode.customerTypeScope || editingCode.customerTypeScope=='ALL'?'selected':''}>Tất cả</option><option value="REGULAR" ${editingCode.customerTypeScope=='REGULAR'?'selected':''}>Khách thường</option><option value="LOYAL" ${editingCode.customerTypeScope=='LOYAL'?'selected':''}>Thân thiết</option><option value="VIP" ${editingCode.customerTypeScope=='VIP'?'selected':''}>VIP</option></select></div>
                                <div class="col-6"><label class="form-label fw-semibold">Sản phẩm áp dụng</label><input class="form-control text-uppercase" name="productCode" value="<c:out value='${editingCode.productCode}'/>" placeholder="VD: SP001"></div>
                        ...
                        <div class="table-responsive"><table class="table table-hover align-middle mb-0">
                            ...
                            <c:forEach items="${codes}" var="code">
                                <tr class="${code.active?'':'opacity-75'}">
                                    ...
                                    <td><div>Đơn từ <fmt:formatNumber value="${code.minimumOrder}" pattern="#,##0"/> đ</div><small class="text-muted">Nhóm: ${code.customerTypeScope=='VIP'?'VIP':code.customerTypeScope=='LOYAL'?'Thân thiết':code.customerTypeScope=='REGULAR'?'Khách thường':'Tất cả'}</small><c:if test="${not empty code.productCode}"><small class="d-block text-muted">SP: <c:out value="${code.productCode}"/> - <c:out value="${code.productName}"/></small></c:if></td>
