...
- PostgreSQL 17 hoặc phiên bản tương thích nếu muốn chạy database local.
  Nếu dùng chung PostgreSQL Render đã có sẵn dữ liệu thì không cần cài PostgreSQL trên máy mới.
- IntelliJ IDEA Ultimate để cấu hình Tomcat trực tiếp.

## Quick setup cho người mới pull code dùng chung DB Render

Đây là cách khuyến nghị cho bài này: máy mới chỉ chạy app, còn dữ liệu dùng chung
PostgreSQL Render đã được khởi tạo sẵn.

Không cần:

- cài PostgreSQL local;
- tạo database mới;
- chạy lại migration V1–V16;
- chạy lại seed data.

Chỉ cần:

1. Cài **JDK 17+**, **Tomcat 10.1.x** và **Maven**.
2. Clone project:
...
4. Chạy bằng Tomcat 10.1 với context:

```text
/grocery-store
```

5. Cấu hình VM options để app trỏ tới Render PostgreSQL:

```text
-Ddb.url=jdbc:postgresql://<RENDER_EXTERNAL_HOST>:5432/<DATABASE>?sslmode=require -Ddb.user=<DB_USER> -Ddb.password=<DB_PASSWORD>
```

Ví dụ format từ Render External Database URL:
...
Không commit hoặc gửi công khai `DB_PASSWORD`, payOS key hay connection string thật.
...
Tài khoản demo trên DB Render/demo hiện tại:
...
Nếu muốn máy mới chạy database riêng/local thay vì Render DB chung, xem mục
“Khởi tạo PostgreSQL local hoặc database mới” bên dưới.

## 1. Khởi tạo PostgreSQL local hoặc database mới

Chỉ làm mục này khi bạn muốn tạo database riêng. Nếu dùng DB Render đã có sẵn,
...
6. Điền cấu hình database vào ô **VM options**.

Nếu dùng chung PostgreSQL Render đã có sẵn dữ liệu:

```text
-Ddb.url=jdbc:postgresql://<RENDER_EXTERNAL_HOST>:5432/<DATABASE>?sslmode=require -Ddb.user=<DB_USER> -Ddb.password=<DB_PASSWORD>
```

Nếu chạy database local tự tạo:
...
```text
DB_URL=jdbc:postgresql://<RENDER_EXTERNAL_HOST>:5432/<DATABASE>?sslmode=require
DB_USER=grocery_app
...
docker run --rm -p 8081:8080 ^
  -e DB_URL="jdbc:postgresql://<RENDER_EXTERNAL_HOST>:5432/<DATABASE>?sslmode=require" ^
  -e DB_USER="<DB_USER>" ^
